    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-5 animate-slide-in">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs">{{ isset($santri) ? 'Perbarui informasi santri' : 'Input data santri pesantren' }}</p>
                </div>
                <div class="flex gap-2 items-center">
                    <!-- Settings Dropdown -->
                    <div class="relative" id="settingsCreate">
                        <button onclick="toggleDropdown('settingsCreate')" class="bg-slate-600 text-white px-3 py-2 rounded-lg font-semibold hover:bg-slate-700 shadow text-xs transition-smooth flex items-center gap-1.5">
        ...
        <!-- Form Card -->
        <form method="POST" action="{{ isset($santri) ? route('santri.update', $santri->id) : route('santri.store') }}" class="bg-white rounded-xl shadow-lg p-6 animate-slide-up">
            @csrf
            @if(isset($santri))
                @method('PUT')
            @endif
            ...
            <div class="mt-5 flex gap-3">
                <button type="submit" class="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm hover-lift shadow-lg transform transition-smooth flex items-center justify-center gap-2">
                    ...
                    {{ isset($santri) ? 'Perbarui Santri' : 'Simpan Santri' }}
                </button>
