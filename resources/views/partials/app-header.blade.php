{{-- ============================================================
     ...
     Header 2 : profil pengguna (sticky top-0) + hamburger + panel menu
     ============================================================ --}}
...
<header class="bg-white border-b border-slate-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3">
        ...
        <div class="min-w-0">
            ...
            {{-- Ubah nama pondok pada baris di bawah ini --}}
            <h1 class="text-base sm:text-xl font-extrabold text-slate-900 leading-tight truncate">Pondok Pesantren</h1>
        </div>
    </div>
</header>
...
<div class="sticky top-0 z-[999] bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- ===== PANEL MENU NAVIGASI (slide-down di bawah baris profil) ===== --}}
        <nav id="navPanel" aria-label="Menu navigasi utama"
             class="overflow-hidden transition-all duration-300 ease-in-out max-h-0 opacity-0">
            <div class="py-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 border-t border-slate-100">
                @php
                    $navItems = [
                ...
                @foreach ($navItems as $item)
                    ...
                    <a href="{{ route($item[1]) }}"
                       class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors {{ $active ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
...
{{-- Overlay gelap di bawah header --}}
<div id="navOverlay" onclick="closeNavPanel()" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-[998]"></div>

<script>
    function openNavPanel() {
        var p = document.getElementById('navPanel');
        p.classList.remove('max-h-0'); p.classList.add('max-h-[600px]'); p.classList.remove('opacity-0');
        document.getElementById('navOverlay').classList.remove('hidden');
        document.getElementById('hdrChevron').classList.remove('rotate-180');
        closeHdrAccount();
    }
    function closeNavPanel() {
        var p = document.getElementById('navPanel');
        if (p) { p.classList.remove('max-h-[600px]'); p.classList.add('max-h-0', 'opacity-0'); }
        var o = document.getElementById('navOverlay');
        if (o) { o.classList.add('hidden'); }
    }
    function toggleNavPanel() {
        var p = document.getElementById('navPanel');
        if (p.classList.contains('max-h-0')) { openNavPanel(); } else { closeNavPanel(); }
    }
